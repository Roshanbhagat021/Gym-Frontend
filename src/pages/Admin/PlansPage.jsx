    ...
    <div>
      ...
      <DataTable
        ...
        columns={[
          ...
          { key: 'price', header: 'Base price', render: (row) => <div><p className="font-bold">{currency(row.price)}</p><p className="text-xs text-steel">+ selected GST at billing</p></div> },
...
function PlanForm({ open, plan, onClose, onSaved }) {
  ...
  return <FormModal open={open} title={plan ? 'Edit plan' : 'Add plan'} onClose={onClose}><form onSubmit={handleSubmit(submit)} className="grid gap-4 md:grid-cols-2"><Field label="Name"><Input {...register('name', { required: true })} /></Field><Field label="Duration"><Input type="number" {...register('duration', { required: true })} /></Field><Field label="Base price (before GST)"><Input type="number" step="0.01" {...register('price', { required: true })} /><p className="mt-1 text-xs text-steel">The applicable GST rate is selected when payment is recorded.</p></Field><Field label="Active"><select className="min-h-11 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm dark:border-white/10 dark:bg-[#181a20]" {...register('isActive')}><option value="true">Yes</option><option value="false">No</option></select></Field><Field label="Mark as popular"><select className="min-h-11 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm dark:border-white/10 dark:bg-[#181a20]" {...register('isPopular')}><option value="false">No</option><option value="true">Yes</option></select></Field><div className="md:col-span-2"><Field label="Description"><Textarea {...register('description')} /></Field><p className="mt-2 text-xs text-steel">Marking this plan as popular automatically removes the tag from any other plan.</p><FormActions isSubmitting={formState.isSubmitting} onCancel={onClose} submitLabel="Save plan" /></div></form></FormModal>;
}
